...
const AIBadge = ({ confidence, reasoning, onApply }: { confidence: number; reasoning?: string; onApply?: () => void }) => (
  <Popover>
    <PopoverTrigger asChild>
      <button type="button" className="inline-flex items-center gap-1 rounded-full bg-neutral-100 text-[#000000] border border-black px-1.5 py-px text-[10px] font-medium hover:bg-neutral-200">
      ...
      <DialogContent className="max-w-3xl min-w-[600px] max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="grid gap-1.5">
                <Label className="text-xs flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setAutoExternalId(v => !v)} className={cn("text-[9px] px-1.5 py-px rounded-full border font-medium uppercase", autoExternalId ? "bg-neutral-100 text-black border-black" : "bg-slate-100 text-slate-600 border-slate-200")}>
          ...
          <TabsContent value="economy" className="space-y-3">
            ...
            <div className="grid gap-1.5">
              ...
              {aiSuggestion && !defaultAccountId && (
                <p className="text-[11px] text-black">AI föreslår {aiSuggestion.accountNumber} ({Math.round(aiSuggestion.confidence * 100)}%)</p>
              )}
            </div>
            <div className="grid gap-1.5">
              ...
              <div className="space-y-2">
                {contacts.map(c => (
                  <div key={c.id} className="flex items-start gap-3 border rounded-md p-3">
                    <div className="w-9 h-9 rounded-full bg-neutral-200 text-black flex items-center justify-center text-xs font-medium shrink-0">{initials(c.name) || "??"}</div>
                    ...
                    <div className="flex flex-col gap-1 items-end shrink-0">
                      <button type="button" onClick={() => setPrimary(c.id)} className={cn("inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full border", c.is_primary ? "bg-neutral-100 text-neutral-700 border-neutral-300" : "bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100")}>
                        <Star className={cn("w-3 h-3", c.is_primary && "fill-amber-500 text-neutral-700")} />{c.is_primary ? "Primär" : "Sätt primär"}
                      </button>
          ...
          <TabsContent value="other" className="space-y-3">
            ...
            <div className="flex items-center justify-between border rounded-md p-3">
              ...
              <div className="flex items-center gap-2">
                <span className={cn("text-xs", isActive ? "text-neutral-700" : "text-muted-foreground")}>{isActive ? "Aktiv" : "Inaktiv"}</span>
        ...
        <div className="flex items-center justify-between border-t pt-4 mt-2">
          <div className="text-xs text-muted-foreground flex items-center gap-1.5">
            {name.trim() ? <CheckCircle2 className="w-3.5 h-3.5 text-neutral-700" /> : <AlertTriangle className="w-3.5 h-3.5 text-neutral-700" />}
