    ...
    <section className="py-20 sm:py-28 bg-gradient-to-b from-[#0B1D2A] to-[#000000]">
      <div className="container mx-auto px-4 sm:px-6">
          ...
          {steps.map((s, i) => (
            <div key={s.step} className="flex items-center gap-4 sm:gap-0">
              <div className="flex flex-col items-center text-center w-[200px]">
                <div className="w-14 h-14 rounded-2xl bg-[rgba(0,0,0,0.08)] border border-[rgba(0,0,0,0.15)] flex items-center justify-center mb-3">
                  <s.icon className="w-6 h-6 text-[#000000]" />
                </div>
                <span className="text-[11px] font-semibold text-[#000000] mb-1">Steg {s.step}</span>
              ...
              {i < steps.length - 1 && (
                <div className="hidden sm:block w-16">
                  <div className="h-px bg-gradient-to-r from-[rgba(0,0,0,0.3)] to-transparent" />
                </div>
              )}
            </div>
          ))}
        ...
        <p className="text-center text-[14px] text-white/60 mb-6">
          Första verifikationen skapas inom <span className="text-[#000000] font-semibold tabular-nums">60 sekunder</span>.
        </p>
