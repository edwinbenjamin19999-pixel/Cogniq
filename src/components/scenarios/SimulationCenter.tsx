...
export function SimulationCenter({ result, kpis, targetEbit }: Props) {
  ...
  const KPI = ({ icon: Icon, label, value, sub, tone = "slate" as "slate" | "emerald" | "rose" | "cyan" }: {
    ...
    const toneCls = {
      slate: "text-slate-900", emerald: "text-[#085041]", rose: "text-[#7A1A1A]", cyan: "text-[#0052FF]",
    }[tone];
    return (
        ...
        <div className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="cash" stroke="#0052FF" strokeWidth={2} fill="url(#cashFill)" name="Kassa" />
