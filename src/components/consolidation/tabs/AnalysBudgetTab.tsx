...
const ExecutiveDashboard = ({ kpiData, groupName }: { kpiData: KPIData | null; groupName: string }) => { if (!kpiData) return null;
  ...
  const cards = [
    ...
    { label: "Likviditet", value: kpiData.cashBalance, accent: "#0052FF", icon: BarChart3, sub: `Kassalikviditet: ${kpiData.currentRatio.toFixed(2)}` },
    { label: "Eget kapital", value: kpiData.equity, accent: "#0052FF", icon: Shield, sub: `Soliditet: ${kpiData.soliditet.toFixed(1)}%` },
  ];
        ...
        <CardContent className="p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data}>
              ...
              <Tooltip formatter={(v: number) => `${formatSEK(v)} kr`} />
              <Legend content={<CustomLegend />} />
              <Bar dataKey="revenue" name="Nettoomsättning" fill="#0052FF" radius={[6, 6, 0, 0]} />
              <Bar dataKey="ebit" name="EBIT" fill="#0052FF" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
    ...
    <Card>
      <CardContent className="p-6">
        ...
        <div className="flex flex-col items-center gap-4">
          <KPINode label="ROE" value={`${roe.toFixed(1)}%`} accent="#0052FF" />
          ...
          <div className="flex items-start gap-16">
            ...
            <div className="flex flex-col items-center gap-3">
              <KPINode label="Kapitalrotation" value={`${kapRot.toFixed(2)}x`} accent="#0052FF" />
              <div className="w-px h-4 bg-border" />
              <div className="flex gap-6">
