...
export function AICFOPanel({
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <Card className="p-4 bg-gradient-to-br from-[#0052FF]/10 to-transparent border-[#C8DDF5]">
        <div className="flex items-start gap-2">
          <Bot className="h-5 w-5 text-[#0052FF] mt-0.5" />
          <div>
      ...
      <Card className="p-4">
        <h4 className="text-[11px] uppercase tracking-wide text-muted-foreground mb-3 flex items-center gap-1">
          <Sparkles className="h-3 w-3 text-[#0052FF]" /> Topp 3 åtgärder
        </h4>
        {top3.length === 0 ? (
            ...
            {top3.map((insight) => {
              ...
              return (
                <div key={insight.id} className="rounded-lg border p-3 space-y-2">
                  ...
                  <Button
                    size="sm"
                    className="w-full h-8 text-xs bg-[#0052FF] hover:bg-[#0052FF] text-white"
