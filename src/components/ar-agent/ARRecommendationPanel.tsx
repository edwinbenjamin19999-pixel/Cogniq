...
export const ARRecommendationPanel = ({ openInvoices, customers }: Props) => {
  ...
  return (
    <div className="rounded-2xl border border-border border-l-[3px] border-l-[#0052FF] bg-card text-card-foreground p-6 shadow-sm space-y-5">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div className="h-10 w-10 rounded-xl bg-[#EFF6FF] border border-[#C8DDF5] flex items-center justify-center flex-shrink-0">
            <Sparkles className="h-5 w-5 text-[#0052FF] dark:text-[#1E3A5F]" />
          </div>
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[#0052FF] dark:text-[#1E3A5F] mb-0.5">AI rekommenderar</p>
        ...
        <div className="lg:ml-auto flex-shrink-0">
          <Button
            className="bg-[#0F1F3D] hover:from-[#0052FF] hover:to-blue-500 text-white shadow-[0_2px_8px_rgba(0,82,255,0.25)] h-10"
      ...
      <div className="flex flex-wrap gap-2">
        {recommendations.map((r, i) => {
          ...
          return (
            ...
            >
              <r.icon className="h-3.5 w-3.5 text-[#0052FF] dark:text-[#1E3A5F]" />
              {r.label}
              <span
                className={cn(
                  "ml-1 px-1.5 py-0.5 rounded text-[10px] font-semibold",
                  tone === "cyan" && "bg-[#EFF6FF] text-[#0052FF] dark:text-[#0052FF]",
