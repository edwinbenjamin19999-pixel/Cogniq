...
export const DemoDataBanner = ({ moduleName }: DemoDataBannerProps) => { const [dismissed, setDismissed] = useState(false);
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-neutral-300/50 bg-[#FAEEDA] dark:bg-amber-950/20 dark:border-neutral-700/30 px-4 py-3">
      <AlertTriangle className="h-4 w-4 text-[#7A5417] dark:text-[#C28A2B] shrink-0" />
      <p className="text-sm text-[#7A5417] dark:text-neutral-300 flex-1">
      ...
      <button onClick={() => setDismissed(true)} className="text-[#7A5417] dark:text-[#C28A2B] hover:text-[#7A5417] dark:hover:text-neutral-300">
        <X className="h-4 w-4" />
      </button>
    </div>
  );
