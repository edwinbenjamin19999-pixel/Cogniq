    ...
    <div className="flex-1 flex flex-col items-center justify-center px-6 text-center max-w-2xl mx-auto w-full">
      <div
        ...
        style={{
          ...
          boxShadow: "0 10px 30px -10px rgba(0,82,255,0.5)",
        }}
      >
      ...
      {!hasBank && (
        <div className="w-full mb-6 rounded-xl border border-border bg-card p-4 flex items-center justify-center gap-2">
          <button
            ...
            className="h-9 px-3 rounded-lg bg-[#0052FF] text-white text-[13px] font-medium hover:bg-[#0052FF] transition"
          >
            Anslut bank
          </button>
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full">
        {PROMPTS.map((q) => (
          <button
            key={q}
            onClick={() => onPick(q)}
            className="text-left px-4 py-3 rounded-xl border border-border bg-card text-[13px] text-foreground hover:border-[#0052FF]/50 hover:bg-[#0052FF]/5 transition"
          >
            {q}
          </button>
        ))}
