    ...
    <div className={cn(
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0">
          ...
          {auto && (
            <span className="inline-flex items-center gap-1 mt-1 text-[11px] font-semibold text-[#0052FF] bg-[#EFF6FF] px-2 py-0.5 rounded-full">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {data.primary?.intent === "open" ? (
          <Button size="sm" onClick={data.voucherId ? onApprove : onEdit} className="bg-[#0052FF] hover:bg-[#0052FF] text-white">
            {data.primary.label}
            <ArrowRight className="w-3.5 h-3.5 ml-1" />
          </Button>
        ) : (
          <>
            <Button size="sm" onClick={onApprove} className="bg-[#0052FF] hover:bg-[#0052FF] text-white">
