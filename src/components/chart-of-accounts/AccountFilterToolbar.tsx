    ...
    <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-100 px-8 py-3 flex flex-wrap items-center gap-3">
      ...
      <div className="relative flex-1 max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0052FF]/20 focus:border-[#0052FF] transition-all"
        />
        {search && (
      ...
      <div className="flex gap-1 items-center">
        <Zap className="w-3.5 h-3.5 text-[#0052FF] mr-0.5" />
        {SMART_FILTERS.map(sf => (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-full whitespace-nowrap transition-all bg-white border border-[#C8DDF5] text-[#0052FF] hover:bg-[#EFF6FF] hover:border-[#0052FF]"
          >
            {sf.label}
          </button>
        ))}
      ...
      <select
        ...
        className="text-sm px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 focus:ring-2 focus:ring-[#0052FF]/20 cursor-pointer"
      >
      ...
      <span className="text-xs text-slate-400 ml-auto whitespace-nowrap">
        ...
        {search && <span className="text-[#0052FF] ml-1">— "{search}"</span>}
      </span>
    </div>
  );
}
