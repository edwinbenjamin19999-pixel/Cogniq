...
export function PosSetupFlow() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
      {/* HERO */}
      <div className="rounded-2xl border border-[#C8DDF5] bg-gradient-to-br from-[#0052FF]/10 via-card to-blue-500/10 dark:from-[#0052FF]/15 dark:via-card dark:to-blue-500/15 p-6 md:p-8 shadow-sm animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1 space-y-3">
            <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-[#EFF6FF] text-[#0052FF] dark:text-[#0052FF] border border-[#C8DDF5]">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {[
                ...
                <span key={c.label} className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-card border border-[#C8DDF5] text-[#0052FF] dark:text-[#0052FF]">
          ...
          <div className="md:w-72 shrink-0 rounded-2xl bg-card border border-[#C8DDF5] p-5 text-center">
            ...
            <p className="text-4xl font-bold text-[#0052FF] dark:text-[#1E3A5F] tabular-nums mt-1">95%</p>
            ...
            <Button
              ...
              className="w-full mt-4 bg-[#0052FF] hover:bg-[#0052FF] text-white"
            >
        ...
        {STEP_LABELS.map((s, i) => {
          ...
          return (
            <div key={s.id} className="flex items-center gap-2 shrink-0">
              {i > 0 && <div className={cn("h-px w-6 md:w-10", isPast || isActive ? "bg-[#0052FF]" : "bg-border")} />}
              <div className={cn(
                "flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-colors border",
                isActive ? "bg-[#0052FF] text-white border-blue-600" : isPast ? "bg-[#EFF6FF] text-[#0052FF] dark:text-[#0052FF] border-[#C8DDF5]" : "bg-card text-muted-foreground border-border"
              )}>
        ...
        <div className="space-y-5">
          {step === "provider" && (
            <>
              ...
              {!autoDetectDismissed && (
                <Card className="border-l-[3px] border-l-[#0052FF] bg-[#EFF6FF] animate-fade-in">
                  <CardContent className="pt-5 pb-5 flex items-center gap-3 flex-wrap">
                    <Brain className="h-5 w-5 text-[#0052FF] dark:text-[#1E3A5F] shrink-0" />
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => { setSelectedProvider("zettle"); setStep("mapping"); }} className="bg-[#0052FF] hover:bg-[#0052FF] text-white">Bekräfta</Button>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          selectedProvider === p.id ? "border-[#0052FF] bg-[#EFF6FF] border-l-[3px] border-l-[#0052FF]" : "border-border"
                        )}
                      >
                        {p.badge && (
                          <span className="absolute top-3 right-3 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#0052FF] text-white">
                            {p.badge}
                          </span>
                        )}
                        <div className="flex items-start gap-3">
                          ...
                          <ArrowRight className="h-4 w-4 text-muted-foreground/50 group-hover:text-[#0052FF] dark:group-hover:text-[#1E3A5F] transition-colors mt-1" />
                        </div>
                      </button>
                    );
                  })}
