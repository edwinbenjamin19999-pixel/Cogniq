    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          "shadow-[0_4px_12px_rgba(0,82,255,0.35),_0_2px_4px_rgba(0,0,0,0.15)]",
          "hover:shadow-[0_8px_24px_rgba(0,82,255,0.45)]",
          "hover:[animation:none]",
          "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0052FF]/50",
          open && "hidden"
        )}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b bg-[hsl(185,80%,10%)]">
          <div className="flex items-center gap-2 min-w-0">
            <Sparkles className="h-4 w-4 text-[#0052FF] shrink-0" />
            <span className="text-sm font-semibold text-white truncate">{assistantName}</span>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#0052FF]/20 text-[#0052FF] font-medium truncate">
              {contextLabel}
            </span>
          </div>
          ...
          <div className="py-4 space-y-4">
            {messages.length === 0 && (
              <div className="text-center py-8 space-y-4">
                <div className="mx-auto w-12 h-12 rounded-full bg-[#0052FF]/10 flex items-center justify-center">
            ...
            {messages.map((msg) => (
              <div key={msg.id}>
                <div className={cn("flex gap-2", msg.role === "user" ? "justify-end" : "justify-start")}>
                  {msg.role === "assistant" && (
                    <div className="w-7 h-7 rounded-full bg-[#0052FF] flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <div
                    className={cn(
                      "rounded-xl text-sm",
                      msg.role === "user"
                        ? "max-w-[80%] bg-[#0052FF] text-white px-3 py-2 rounded-br-sm"
                        : "max-w-[90%] bg-card border px-3 py-2 rounded-bl-sm"
                    )}
                  >
                    {msg.role === "assistant" ? (
                      <div className="prose prose-sm dark:prose-invert max-w-none prose-p:text-[12px] prose-p:my-1 prose-li:text-[12px] prose-strong:text-foreground prose-code:text-[#0052FF] prose-code:text-xs">
                ...
                {msg.role === "assistant" && msg.suggestedQuestions && msg.suggestedQuestions.length > 0 && !isLoading && (
                  <div className="flex flex-wrap gap-1.5 mt-2 ml-9">
                    {msg.suggestedQuestions.map((q, i) => (
                      <button
                        key={i}
                        className="text-[11px] px-2.5 py-1 rounded-full border border-[#0052FF]/30 text-[#0052FF] hover:bg-[#0052FF]/10 transition-colors"
        ...
        <div className="border-t p-3 shrink-0">
          ...
          <div className="flex items-end gap-2">
            ...
            <Button
              ...
              size="icon"
              className="rounded-full h-8 w-8 bg-[#0052FF] hover:bg-[#0052FF]/90"
            >
