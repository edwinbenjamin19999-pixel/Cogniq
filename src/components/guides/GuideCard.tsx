...
export const GuideCard = ({ guide }: { guide: Guide }) => (
  <div className="group flex items-start justify-between gap-4 rounded-xl border border-slate-100 bg-white p-5 hover:border-[#C8DDF5] hover:shadow-sm transition-all duration-150">
    <div>
      <h4 className="text-[15px] font-semibold text-[#0F172A] leading-snug group-hover:text-[#0052FF] transition-colors">{guide.title}</h4>
    ...
    <ArrowRight className="w-4 h-4 mt-1 text-[#0052FF] group-hover:translate-x-0.5 transition-transform shrink-0" />
  </div>
);
