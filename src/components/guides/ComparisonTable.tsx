    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b border-slate-100 bg-slate-50/60">
          <th className="text-left font-semibold text-[#000000] px-6 py-4">Funktion</th>
          <th className="text-left font-semibold text-[#64748b] px-6 py-4">Traditionellt system</th>
          <th className="text-left font-semibold text-[#000000] px-6 py-4">Bokfy</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((r, i) => (
          <tr key={i} className="border-b border-slate-100 last:border-0">
            <td className="px-6 py-4 font-medium text-[#000000]">{r.label}</td>
            ...
            <td className="px-6 py-4 text-[#000000]">
              {typeof r.ai === "boolean"
                ? (r.ai ? <Check className="w-4 h-4 text-[#000000]" /> : <X className="w-4 h-4 text-slate-300" />)
                : r.ai}
            </td>
          </tr>
        ))}
