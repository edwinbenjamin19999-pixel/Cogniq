  ...
  return (
    <div className="rounded-2xl border border-[#0052FF]/20 bg-gradient-to-br from-[#0052FF]/5 to-transparent p-5">
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-xl bg-[#0052FF] flex items-center justify-center flex-shrink-0">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {questions.map((q, i) => (
              <button
                key={i}
                onClick={() => ask(q)}
                className="group inline-flex items-center gap-1.5 text-[12px] px-3 py-1.5 rounded-full border border-[#0052FF]/30 bg-white hover:bg-[#0052FF]/10 text-[#0040CC] font-medium transition-colors"
              >
                {q}
