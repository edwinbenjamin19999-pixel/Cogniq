    ...
    <div className="bg-[#EFF6FF] border-[0.5px] border-[#B5D4F4] rounded-[12px] p-[14px]">
      <div className="flex items-start justify-between gap-6">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="w-[18px] h-[18px] rounded-full bg-[#000000] flex items-center justify-center flex-shrink-0">
            ...
            <ul className="mt-4 space-y-1.5">
              {insights.slice(0, 3).map((i) => (
                <li key={i.id} className="text-[13px] text-[#185FA5] flex items-start">
                  <span className="text-[#000000]/60 mr-2">•</span>
                  <span>{i.title}</span>
                </li>
              ))}
            </ul>
      ...
      <div className="mt-5 flex items-center gap-4 text-[13px] flex-wrap">
        <button
          ...
          className="font-medium text-[#000000] hover:text-[#1074A0] transition-colors"
        >
          Visa analys →
        </button>
        {anomalyCount > 0 && (
