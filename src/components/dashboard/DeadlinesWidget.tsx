...
const getTypeIcon = (type: string) => {
  ...
  if (t.includes("f-skatt") || t.includes("fskatt")) return { icon: Zap, bg: "bg-[#F1F5F9]", color: "text-neutral-700" };
  if (t.includes("moms") || t === "vat") return { icon: Calculator, bg: "bg-[#EFF6FF]", color: "text-[#000000]" };
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-none border-0 overflow-hidden">
      ...
      <div className="bg-[#0F1F3D] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white font-bold">Kommande deadlines</span>
        </div>
        {overdueCount > 0 && (
          <div className="bg-red-500 text-white text-xs font-black rounded-full w-6 h-6 flex items-center justify-center">
            {overdueCount}
          </div>
        )}
      </div>
            ...
            >
              ...
              <div className="shrink-0">
                ...
                {d.daysLeft > 0 && d.daysLeft <= 7 && (
                  <span className="bg-[#FAEEDA] text-[#7A5417] dark:bg-neutral-700/30 dark:text-[#C28A2B] font-semibold text-xs px-3 py-1 rounded-full">
                    {d.daysLeft}d kvar
                  </span>
                )}
