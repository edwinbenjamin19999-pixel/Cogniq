  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F5F7FA] p-6">
      <div className="bg-white border-[0.5px] border-[#DFE4EA] rounded-[16px] p-8 max-w-md w-full text-center space-y-4">
        {state === "working" && (
          <Loader2 className="h-8 w-8 text-[#000000] mx-auto animate-spin" />
        )}
        {state === "success" && (
          <CheckCircle2 className="h-8 w-8 text-neutral-700 mx-auto" />
        )}
        {state === "error" && (
          <AlertTriangle className="h-8 w-8 text-neutral-700 mx-auto" />
        )}
