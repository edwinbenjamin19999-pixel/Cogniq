    ...
    <>
      ...
      <div className="px-8 pb-16 max-w-[840px] mx-auto space-y-8">
            ...
            <section key={section.title} className="bg-white rounded-3xl border-[0.5px] border-slate-200 p-7">
              <header className="flex items-start gap-3 mb-4">
                <div className="w-9 h-9 rounded-xl bg-[#0052FF]/10 flex items-center justify-center flex-shrink-0">
                  <Icon className="h-4.5 w-4.5 text-[#0052FF]" />
                </div>
                <div>
              ...
              <ul className="space-y-3.5">
                {section.items.map((item) => (
                  <li key={item.label} className="flex items-start gap-3">
                    <CheckCircle2 className="h-4 w-4 text-[#0052FF] mt-0.5 flex-shrink-0" />
        ...
        <p className="text-[12px] text-slate-500 text-center pt-2">
          Saknar du något här? <a href="/contact" className="text-[#0052FF] underline decoration-dotted underline-offset-2">Hör av dig</a> så uppdaterar vi sidan.
        </p>
      </div>
    </>
  );
